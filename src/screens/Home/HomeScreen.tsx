...
import { searchSongs, searchAlbums, searchArtists, searchPlaylists } from '../../api/search.api';
...
// Sample Song ID for suggestions 
...
const HomeScreen = () => {
  ...
  const [searchResultsArtists, setSearchResultsArtists] = useState<Artist[]>([]);
  const [searchResultsAlbums, setSearchResultsAlbums] = useState<Album[]>([]);
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <SongCard
                ...
                onPress={() => handleSongPress(item, searchResultsSongs)}
              />
            )}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <AlbumCard
                  ...
                  image={{ uri: item.image[2]?.url || item.image[0]?.url }}
                  onPress={() =>
                    (navigation as any).navigate('AlbumDetails', {
                      id: item.id,
                      albumData: item,
                    })
                  }
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      setSearchResultsArtists(artists);
      setSearchResultsAlbums(albums);
      setSearchResultsPlaylists(playlists);
    } catch (err) {
